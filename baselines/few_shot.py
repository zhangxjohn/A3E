...
from prompts import SYSTEM_PROMPT, FEW_SHOT_PROMPT, EXAMPLE1, EXAMPLE2, EXAMPLE3
...
def main(all_analogies, args, dirname):
    ...
    for sample_data in all_analogies:
        try:
            ...
            random_demos = random.sample(EXAMPLES, args.k_shot_demos)
            random.shuffle(random_demos)
            K_SHOT_DEMOS = ''
            for demo in random_demos:
                K_SHOT_DEMOS += demo
            zero_prompt = FEW_SHOT_PROMPT.format(base=base, target=target, few_shots=K_SHOT_DEMOS)
            tries = 0
            max_tries = 5
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--k_shot_demos', '-KSD', type=int, default=1, help='the number of few shot')
